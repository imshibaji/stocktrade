...
<head>
    ...
    <title>Error | <?= esc($errorSiteName) ?></title>
    <script>
    (function () {
        try {
...
<body class="bg-page text-ink min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-4xl">
        <div class="text-center mb-8">
            <i class="fas fa-<?= $icon ?? 'exclamation-circle' ?> text-<?= $color ?? 'yellow' ?>-500 text-8xl opacity-80"></i>
        </div>
        <h1 class="text-6xl font-bold text-ink mb-4 text-center"><?= esc($code ?? 'Error') ?></h1>
        <h2 class="text-2xl text-ink-2 mb-6 text-center"><?= esc($title ?? 'An Error Occurred') ?></h2>
        <p class="text-ink-4 max-w-md mx-auto mb-8 text-center">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            ...
            <a href="/" class="text-ink-4 hover:text-ink text-sm self-center">
                <i class="fas fa-home mr-1"></i>Back to Home
            </a>
        </div>
        
        <?php if (ENVIRONMENT !== 'production' && isset($exception)): ?>
        <div class="text-left p-4 bg-surface rounded-lg border border-edge overflow-hidden">
            <p class="text-red-400 text-xs mb-2">Debug Information (Development Only)</p>
            <pre class="text-xs text-ink-3 overflow-x-auto whitespace-pre-wrap"><?= esc((string)$exception) ?></pre>
        </div>
        <?php endif; ?>
    </div>
</body>
